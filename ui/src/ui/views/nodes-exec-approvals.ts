...
function formatSecurityLabel(value: ExecSecurity) {
  switch (value) {
    case "allowlist":
      return t("nodesPage.approvals.securityAllowlist");
    case "full":
      return t("nodesPage.approvals.securityFull");
    case "deny":
    default:
      return t("nodesPage.approvals.securityDeny");
  }
}
...
function formatAskLabel(value: ExecAsk) {
  switch (value) {
    case "always":
      return t("nodesPage.approvals.askAlways");
    case "on-miss":
      return t("nodesPage.approvals.askOnMiss");
    case "off":
    default:
      return t("nodesPage.approvals.askOff");
  }
}
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${t("nodesPage.approvals.title")}</div>
          <div class="card-sub">${t("nodesPage.approvals.subtitle")}</div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.saving ? t("nodesPage.saving") : t("nodesPage.save")}
        </button>
      </div>
      ...
      ${
        !ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodesPage.approvals.loadPrompt")}</div>
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? t("nodesPage.loading") : t("nodesPage.approvals.load")}
            </button>
          </div>`
          : html`
...
function renderExecApprovalsTarget(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesPage.approvals.targetTitle")}</div>
          <div class="list-sub">${t("nodesPage.approvals.targetSubtitle")}</div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodesPage.approvals.hostLabel")}</span>
            <select
            ...
            >
              <option value="gateway" ?selected=${state.target === "gateway"}>
                ${t("nodesPage.approvals.gateway")}
              </option>
              <option value="node" ?selected=${state.target === "node"}>
                ${t("nodesPage.approvals.node")}
              </option>
            </select>
          </label>
          ${
            state.target === "node"
              ? html`
                <label class="field">
                  <span>${t("nodesPage.approvals.nodeLabel")}</span>
                  <select
                  ...
                  >
                    <option value="" ?selected=${nodeValue === ""}>
                      ${t("nodesPage.approvals.selectNode")}
                    </option>
      ...
      ${
        state.target === "node" && !hasNodes
          ? html`
              <div class="muted">${t("nodesPage.approvals.noNodeTargets")}</div>
            `
          : nothing
      }
    </div>
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesPage.approvals.securityTitle")}</div>
          <div class="list-sub">
            ${
              isDefaults
                ? t("nodesPage.approvals.securityHelp")
                : t("nodesPage.approvals.defaultValue", {
        ...
        <div class="list-meta">
          <label class="field">
            <span>${t("nodesPage.approvals.modeLabel")}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
            ...
            >
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${t("nodesPage.approvals.useDefault", {
                      value: formatSecurityLabel(defaults.security),
                    })}
                  </option>`
                  : nothing
              ...
              ${SECURITY_OPTIONS.map(
                (option) =>
                  html`<option value=${option} ?selected=${securityValue === option}>
                    ${formatSecurityLabel(option)}
                  </option>`,
              )}
            </select>
          </label>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesPage.approvals.askTitle")}</div>
          <div class="list-sub">
            ${
              isDefaults
                ? t("nodesPage.approvals.askHelp")
                : t("nodesPage.approvals.defaultValue", {
        ...
        <div class="list-meta">
          <label class="field">
            ...
            >
              ...
              ${ASK_OPTIONS.map(
                (option) =>
                  html`<option value=${option} ?selected=${askValue === option}>
                    ${formatAskLabel(option)}
                  </option>`,
              )}
            </select>
          </label>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesPage.approvals.askFallbackTitle")}</div>
          <div class="list-sub">
            ${
              isDefaults
                ? t("nodesPage.approvals.askFallbackHelp")
                : t("nodesPage.approvals.defaultValue", {
        ...
        <div class="list-meta">
          <label class="field">
            <span>${t("nodesPage.approvals.fallbackLabel")}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesPage.approvals.autoAllowTitle")}</div>
          <div class="list-sub">
            ${
              isDefaults
                ? t("nodesPage.approvals.autoAllowHelp")
                : autoIsDefault
                  ? t("nodesPage.approvals.autoAllowInherited", {
                      value: defaults.autoAllowSkills
                        ? t("common.enabled")
                        : t("common.disabled"),
                    })
                  : t("nodesPage.approvals.autoAllowOverride", {
                      value: autoEffective ? t("common.enabled") : t("common.disabled"),
                    })
            }
          </div>
        </div>
        <div class="list-meta">
          ...
          ${
            !isDefaults && !autoIsDefault
              ...
              >
                ${t("nodesPage.approvals.useDefaultAction")}
              </button>`
              : nothing
          }
        </div>
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        <div class="card-title">${t("nodesPage.approvals.allowlistTitle")}</div>
        <div class="card-sub">${t("nodesPage.approvals.allowlistSubtitle")}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
      ...
      >
        ${t("nodesPage.approvals.addPattern")}
      </button>
    </div>
    <div class="list" style="margin-top: 12px;">
      ${
        entries.length === 0
          ? html`
              <div class="muted">${t("nodesPage.approvals.allowlistEmpty")}</div>
            `
    ...
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">
          ${entry.pattern?.trim() ? entry.pattern : t("nodesPage.approvals.newPattern")}
        </div>
        <div class="list-sub">${t("nodesPage.approvals.lastUsed", { time: lastUsed })}</div>
        ${lastCommand ? html`<div class="list-sub mono">${lastCommand}</div>` : nothing}
        ${lastPath ? html`<div class="list-sub mono">${lastPath}</div>` : nothing}
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t("nodesPage.approvals.patternLabel")}</span>
          <input
            type="text"
            .value=${entry.pattern ?? ""}
            ?disabled=${state.disabled}
        ...
        >
          ${t("nodesPage.approvals.removePattern")}
        </button>
      </div>
    </div>
  `;
