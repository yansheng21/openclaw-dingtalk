...
export async function runReplyAgent(params: {
  ...
  try {
    ...
    if (autoCompactionCount > 0) {
      ...
      // Keep compaction as an internal/control-UI concern. We still persist the
      // updated count and inject post-compaction context, but we do not emit extra
      // user-facing reply payloads to external channels or chat transcripts.
    }
